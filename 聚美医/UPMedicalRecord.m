...
//    [_bigscrollview addSubview:tianjiaxiangmu];
    ...
    _imagelins = [[UIImageView alloc] initWithFrame:CGRectMake(5, 140, self.view.bounds.size.width - 10, 2)];
    _imagelins.image = [UIImage imageNamed:@"hengxian"];
    [_bigscrollview addSubview:_imagelins];
    
    
    _binglitu = [[UILabel alloc] initWithFrame:CGRectMake(10, 155, 80, 20)];
    _binglitu.text = @"病历图片:";
...
//    [_upimage addTarget:self action:@selector(upimage) forControlEvents:UIControlEventTouchUpInside];
    ...
    self.imagessary = [[NSMutableArray alloc] initWithCapacity:0];
...
-(void)addimagepis
{
    ...
    for (int c = 0; c < self.imagessary.count + 1; c++) {
        UIButton *btn = (UIButton *)[_bigscrollview viewWithTag:10 + c];
    ...
    for (int j = 0; j < [self.imagessary count]; j ++) {
        ...
        [button setBackgroundImage:[self.imagessary objectAtIndex:j] forState:UIControlStateNormal];
    ...
    NSInteger a1 = [self.imagessary count];
    ...
    NSInteger a2 = [self.imagessary count] + 1;
...
-(void)cancelbuttonclickl:(UIButton *)btn
{
    ...
    [self.imagessary removeObjectAtIndex:btn.tag - 10];
    [self.imagedataary removeObjectAtIndex:btn.tag - 10];
    [btn removeFromSuperview];
    
    
    [self addimagepis];
    
    for (int j = 0; j < [self.imagessary count] + 1; j ++) {
        
        UIImageView *imagev = (UIImageView *)[_bigscrollview viewWithTag:100 + j];
        [imagev removeFromSuperview];
        
        UIButton *btn = (UIButton *)[_bigscrollview viewWithTag:10 + j];
...
-(void)cancelbtnclick
{
    ...
    if (self.iscancel == NO) {
        ...
        for (int j = 0; j < [self.imagessary count]; j ++) {
            ...
            imagev.tag = 100 + j;
            [_bigscrollview addSubview:imagev];
...
//    [_upimage setTitle:@"" forState:UIControlStateNormal];
    ...
    [self.imagessary addObject:imagess];
    ...
    [self.imagedataary addObject:_datas];

    
}

#pragma mark-----上传病历前获取medicalhistorysno
...
{
    _queding.userInteractionEnabled = NO;
    ...
    for (int i = 0; i < self.imagedataary.count; i++) {
        ...
        [AFHTTPRequestOpeartionManagerOfme postModifyTheUserHeadRequestWitHUser:urlstr medicalhistorysno:self.medicalhistorysno doctorsno:self.doctorsno and:[self.imagedataary objectAtIndex:i] Completion:^(NSMutableArray *array1, NSMutableArray *array2, NSString *string) {
            
            if (self.imagedataary.count == a) {
                _queding.userInteractionEnabled = YES;
                NSLog(@"-------------请求玩啦---------");
            }
