...
-(id)initWithFrame:(CGRect)frame andwidth:(CGFloat)width tyfrom:(NSString *)tyfrom andcontents:(NSString *)contents creatdate:(NSString *)creatdate andimages:(NSData *)imagesData filetype:(NSString *)filetype{
    ...
    if (self) {
        ...
        if ([tyfrom isEqualToString:@"kehu"]) {
            ...
            self.lefetimage.image = [[UIImage imageNamed:@"chatfrom_bg_left"] resizableImageWithCapInsets:ed];
            ...
            if ([filetype isEqualToString:@"20150213142921851"]) {
                ...
                UILabel *date = [[UILabel alloc] initWithFrame:CGRectMake(20, self.lefetcontents.bounds.size.height + self.lefetcontents.frame.origin.y + 2, self.bounds.size.width, 20)];
                date.text = creatdate;
                date.textColor = [self colorWithRGB:0x999999 alpha:1];
                date.font = [UIFont systemFontOfSize:14];
                [self.lefetimage addSubview:date];
            ...
            UIEdgeInsets ed = {50.0f, 40.0f, 40.0f, 40.0f};
            self.rigthimage.image = [[UIImage imageNamed:@"chatfrom_bg_ritht"] resizableImageWithCapInsets:ed];
            ...
            if ([filetype isEqualToString:@"20150213142908837"]) {//文字
                ...
                self.rigthcontents.frame = CGRectMake(10, 12, self.rigthimage.bounds.size.width - 30, contentshigth);
            ...
            if ([filetype isEqualToString:@"20150213142921851"]) {
                ...
                UILabel *date = [[UILabel alloc] initWithFrame:CGRectMake(10, self.rigthcontents.bounds.size.height + self.rigthcontents.frame.origin.y + 2, self.bounds.size.width, 20)];
                date.text = creatdate;
                date.textColor = [self colorWithRGB:0xffffff alpha:1];
                date.font = [UIFont systemFontOfSize:14];
                [self.rigthimage addSubview:date];
